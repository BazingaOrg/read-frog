...
function TranslateFooterContent(props: ComponentProps<typeof SelectionToolbarFooterContent>) {
  return <SelectionToolbarFooterContent {...props} />
}
...
export function SelectionTranslationProvider({ children }: { children: ReactNode }) {
  ...
  const translateProviders = useMemo(
    () => getSelectableProvidersForCapability("translation", providersConfig),
    [providersConfig],
  )
  ...
  const handleProviderChange = useCallback(
    (providerId: string) => {
      void setConfig(buildFeatureProviderPatch({ translation: providerId }))
    },
    [setConfig],
  )
  ...
  const handleOpenChange = useCallback(
    ...
    [applyPendingSession, cancelCurrentTranslation, resetPopoverSession, resetTranslationState],
  )
  ...
  const handleReuseRequest = useCallback(() => {
    ...
    setRerunNonce((prev) => prev + 1)
  }, [applyPendingSession, cancelCurrentTranslation, resetTranslationState])
