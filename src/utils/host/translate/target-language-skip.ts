 ...
 *
 * Runs BEFORE any wrapper/spinner DOM insertion: detection is local franc, so
 * same-language paragraphs can be skipped without the insert-then-remove DOM
 * churn and spinner flash they previously paid. Texts under the length
 * threshold are never skipped — franc is unreliable on short input.
 */
